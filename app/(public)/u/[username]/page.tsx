...
import { Cpu, Eye, Settings2, Terminal } from "lucide-react";
...
import { CourseProgressCard } from "@/components/perfil/course-progress-card";
...
export default async function PerfilPublicoPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const [followersCount, followingCount, caletasCount, followRow, recursos, aprendeProgress] = await Promise.all([
    ...
    getAprendeProgressForUser(user.id),
  ]);

  const gallery = parseProfileGalleryUrls(user.profileGalleryUrls);
  const isFollowing = !!viewerId && !isOwnProfile && !!followRow;
  const pic18Summary = summarizePic18Progress(aprendeProgress.pic18.payload);
  const cppSummary = summarizeCppPooProgress(aprendeProgress.cpp.payload);

  return (
    <PublicPageShell>
      <div className="chalk-container min-w-0 py-6 sm:py-8">
      <div className="chalk-card overflow-hidden">
        <div className="relative h-36 sm:h-44">
          {user.profileBannerUrl ? (
            ...
            <div className="h-full w-full bg-gradient-to-r from-[#1C2D20] via-[#2a3d32] to-[color-mix(in_oklab,var(--caleta-accent)_25%,#1C2D20)]" />
          )}
          ...
          <div className="-mt-14 flex flex-col gap-4 sm:-mt-16 sm:flex-row sm:items-end sm:justify-between">
            ...
            {!isOwnProfile && viewerId ? (
              ...
              <Link
                ...
                className="inline-flex items-center justify-center gap-2 self-center rounded-lg border border-[color-mix(in_oklab,var(--caleta-accent)_45%,transparent)] bg-[#1C2D20] px-4 py-2 text-sm text-[var(--caleta-accent)] hover:bg-white/10 sm:self-end"
              >
      ...
      {pic18Summary ? (
        <CourseProgressCard
          icon={Cpu}
          title="Aprende PIC18"
          subtitle="Microcontroladores PIC18F4550 · progreso sincronizado con CALETAS"
          href="https://pic18.caleta.top"
          summary={pic18Summary}
          updatedAt={aprendeProgress.pic18.updatedAt}
        />
      ) : null}
      ...
      {cppSummary ? (
        <CourseProgressCard
          icon={Terminal}
          title="AprendeC++ POO"
          subtitle="Computacion II · C++ y POO · progreso sincronizado con CALETAS"
          href="https://cpp.caleta.top"
          summary={cppSummary}
          updatedAt={aprendeProgress.cpp.updatedAt}
        />
      ) : null}
      ...
      <section className="mt-10">
        ...
        {recursos.length === 0 ? (
          <p className="rounded-xl border border-white/10 bg-[color-mix(in_srgb,var(--surface-elevated)_90%,black)] px-4 py-8 text-center text-white/60">
            {isOwnProfile
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-3 md:grid-cols-4">
            {recursos.map((r) => (
              <Link
                ...
                className="group flex flex-col overflow-hidden rounded-xl border border-white/10 bg-[color-mix(in_srgb,var(--surface-elevated)_92%,black)] transition hover:border-[color-mix(in_oklab,var(--caleta-accent)_40%,transparent)] hover:bg-[#3d5649]"
              >
