...
/** GET — recursos publicos de CALETAS relacionados con Computacion II / C++ POO */
export async function GET(request: Request) {
  try {
        ...
        OR: [
          {
            materia: {
              OR: [
                { codigo: "ABI4222" },
                { nombre: { contains: "Computación II", mode: "insensitive" } },
                { nombre: { contains: "Computacion II", mode: "insensitive" } },
              ],
            },
          },
          ...CPP_TAG_FRAGMENTS.map((tag) => ({
    ...
    console.error("Error fetching aprende-cpp-poo recursos:", error);
